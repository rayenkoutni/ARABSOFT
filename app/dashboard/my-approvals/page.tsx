  ...
  useEffect(() => {
    const loadRequests = async () => {
      ...
      try {
        ...
        if (requestId) {
          const target = data.find((request) => request.id === requestId && canUserExamineRequest(request, user.role))
          if (target) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1" style={{ color: 'var(--color-text-muted)' }}>
          Examinez et approuvez les demandes en attente de votre equipe
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {visibleRequests.map((request) => {
            const isActionable = canUserExamineRequest(request, user.role)
